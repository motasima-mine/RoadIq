...
- 2026-08-17 09:50 — merged experimental into main, excluding voice_server.py / static/voice.html / static/voice-worklet.js / scripts/test_voice_server.py (Nova Sonic voice prototype, blocked by network HTTP/2 issue, not code-ready); pulled in dynamic corridor bbox, route-aware chat/competitors, Fleet Intelligence, PFJ 360 food, model routing (Nova Pro), and the Gold->Elite wording fix. Post-merge verification: /api/driver, /api/plan (Nashville->Dallas long route), /api/ai chat, /api/competitors, /api/fleet all returning correct real data.
